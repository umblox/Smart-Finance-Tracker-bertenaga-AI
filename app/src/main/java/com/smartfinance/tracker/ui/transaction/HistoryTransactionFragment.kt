...
class HistoryTransactionFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val root = LinearLayout(context).apply { orientation = LinearLayout.VERTICAL; setBackgroundColor(Color.parseColor("#F7FAFC")) }
        ...
        // NAVIGASI PENELUSUR BULAN KAS SEJARAH
    ...
    private fun refreshHistoryEngine() {
        ...
        lifecycleScope.launch {
            ...
            // FILTER BERDASARKAN RENTANG BULAN YANG DIPILIH
            ...
            if (monthlyTx.isEmpty()) {
                transactionListContainer.addView(TextView(context).apply { text = "\nBelum ada rekam jejak keuangan di bulan ini."; gravity = Gravity.CENTER; setTextColor(Color.GRAY) })
                return@launch
            }
            ...
            monthlyTx.forEach { tx ->
                ...
                if (!groupMap.containsKey(dayString)) {
                    groupMap[dayString] = ArrayList()
                }
                groupMap[dayString]?.add(tx)
            }

            groupMap.forEach { (dateHeader, transactions) ->
                // KARTU INDUK PER HARI
                ...
                // Header Tanggal Hari
                val tvDayHeader = TextView(context).apply { text = dateHeader.uppercase(); textSize = 11f; setTypeface(null, Typeface.BOLD); setTextColor(Color.parseColor("#4A5568")) }
                ...
                // Iterasi Transaksi di Tanggal Tersebut
